...
# ฟังก์ชันเดินตรงตามแกนด้วย PD-Control
def move_straight(ep_chassis, axis='x', distance=1.2):
    global current_x, current_y
    print(f"Moving {distance} meters on {axis.upper()} axis using PD-Control...")
    ...
    while True:
        ...
        # อัปเดตค่า prev_error
        prev_error = error

        # เงื่อนไขหยุดเมื่อใกล้ถึงเป้าหมาย
        if abs(error) <= 0.01:
        ...
        print(f"Error: {error:.3f}, Derivative: {d_error:.3f}, Speed: {speed:.3f}")
        time.sleep(dt)
...
if __name__ == '__main__':
    ...
    try:
        print("== Start Square Movement (PD-Control) ==")
